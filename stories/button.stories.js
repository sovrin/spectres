...
const propTypes = Object
    .keys(Button.propTypes)
    .map(key => {

        console.info(Button.propTypes[key], key);

        switch (Button.propTypes[key]){
            case PropTypes.bool: {
                boolean(key, false);
                break;
            }
            ...
            case oneOfConstants: {
                ...
                // select(key, Button[key.charAt(0).toUpperCase() + key.slice(1)]);
                break;
            }
...
stories
    .add('Sizes', () =>
            {/*<Button size={select('Size', Button.Size)}/>*/}


        // button({}, 'default'),
        // button({primary: true}, 'primary'),
        // button({link: true}, 'link'),
        // ]
    )
;
